...
router.get('/category/:category', async (req, res) => {
  ...
  if (cached) return res.json(cached);
  try {
    const result = await queries.getDealsByCategory(category, { platform, sub_category, sort, limit: Math.min(parseInt(limit)||40, 80) });
...
// GET /api/deals/stats — must come BEFORE /:id
...
router.post('/:id/report-coupon', async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: 'Could not report' }); }
});
...
router.post('/:id/vote', async (req, res) => {
  const { id } = req.params;
  const { vote } = req.body;
  if (!['active','expired'].includes(vote)) return res.status(400).json({ error: 'Vote must be active or expired' });
  const ipHash = hashIp(req.ip || 'unknown');
  try {
